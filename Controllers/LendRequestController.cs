...
namespace LibraryManagmentSystem.Controllers
{
    public class LendRequestController : Controller
    {
        ...
        public ViewResult ReturnBook(int lendId)
        {
            ...
            int diff = (int)(date.CompareTo(lendRequest.ReturnDate));
            ...
            connect = "Server=localhost;Database=LMSdb;Trusted_Connection=True";
            ...
            sql = $"Select LendDate From LendRequests Where LendId={lendId}";
            command = new SqlCommand(sql, cnn);

            output = (DateTime)command.ExecuteScalar();
            command.Dispose();

            int diff = ((int)(date - output).TotalDays) +1;
            float fine = diff * 10;

            sql = $"Insert FineAmount={fine} into LendRequests  where LendId={lendId}";
